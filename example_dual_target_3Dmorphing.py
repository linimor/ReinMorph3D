...
for tmp_name in [
    ["typical_creature_elephant.png", "typical_vehicle_excavator.png", "Christmas_tree.png"]
    ]:
    ...
    dual_img_path_list.append(f"./assets/example_morphing/{tmp_name[2]}")
...
for idx in range(len(src_img_path_list)):
    ...
    morphing_params = {"save_cache_path": dual_tar_save_cache_path, "init_morphing_flag": False, "ss_mca_flag": False, "slat_mca_flag": False, "ss_tfsa_flag": False, "slat_tfsa_flag": False, "oc_flag": False}
    if not os.path.exists(f"{dual_tar_save_cache_path}/slat_init.pt"):
        run_morphing_cache(pipeline, dual_tar_img, src_img, morphing_params, SEED, dual_tar_save_path, dual_tar_name)

    name = src_name + "+" + tar_name

    morphing_params = {"morphing_num": 50, "src_load_cache_path": src_save_cache_path, "tar_load_cache_path": tar_save_cache_path, "init_morphing_flag": False, "ss_mca_flag": True, "slat_mca_flag": True, "ss_tfsa_flag": True, "slat_tfsa_flag": True, "oc_flag": False}
    save_path = os.path.join(save_dir_path, "dual_target_3Dmorphing", name)
    ...
    morphing_params["dual_tar_img"] = dual_tar_img
    morphing_params["dual_tar_load_cache_path"] = dual_tar_save_cache_path
