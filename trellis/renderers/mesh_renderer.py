...
class MeshRenderer:
    ...
    def render(
        ...
        bg_color = self.rendering_options["bg_color"]
        ...
        for type in return_types:
            ...
            if type == "mask" :
                ...
                if True:
                    R = extrinsics[:3, :3].unsqueeze(0)
                    img = torch.matmul(img, R.transpose(-1, -2))
            ...
            if type == "normal" or type == "normal_map" or type == "color":
                tmp_bg_color = torch.tensor(bg_color, dtype=torch.float32, device=self.device)
                tmp_bg_color = torch.ones_like(img) * tmp_bg_color
                img = torch.lerp(tmp_bg_color, img, mask.float())
